...
// WireCanvas children:
//   [0,       kLats)           back lat Polylines   (9)
//   [kLats,   2*kLats)         front lat Polylines  (9)
//   [2*kLats, 2*kLats+kLons)   lon Polylines       (12)
...
void GlobeGridBackground::InitItems()
{
    ...
    // Back hemisphere lat Polylines (dim)
    ...
    // Front hemisphere lat Polylines (bright; equator in cyan)
    for (int i = 0; i < kLats; ++i) {
        auto pl = ref new Polyline();
        if (i == 4) {
    ...
    // Lon Polylines (opacity toggled per frame by hemisphere)
    ...
    // Stars: distributed across the full canvas in circular orbits
    ...
    for (int i = 0; i < kStars; ++i) {
        ...
        // Slower orbits for outer stars, slightly faster for inner
...
{
    // Back lat arcs: lon_view from π/2 → 3π/2
    for (int i = 0; i < kLats; ++i) {
    ...
    // Front lat arcs: lon_view from -π/2 → π/2
    for (int i = 0; i < kLats; ++i) {
        float latR = kLatDeg[i] * kPi / 180.0f;
    ...
    // Lon lines: front vs back determined by view-space angle
    for (int j = 0; j < kLons; ++j) {
...
void GlobeGridBackground::UpdateStars()
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // Gentle twinkle: opacity pulses slowly with orbit phase
