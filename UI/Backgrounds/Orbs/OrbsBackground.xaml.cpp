...
// Default glow / background — "electric" scheme
...
static const float kOuterAuraR    = 40.0f;   // soft outer bloom
static const float kInnerGlowR    = 30.0f;   // tight bright halo
static const float kCoreR         = 20.0f;   // sharp white point
static const float kMinTailR      = 1.5f;
static const float kMaxTailR      = 9.0f;
static const float kMaxTailOpacity = 0.88f;

static const float kCircleTilt    = 20.0f * kPi / 180.0f;  // slight 3D tilt of the formation circle
static const float kCircleRotSpeed = 0.007f;
static const int   kDanceHoldTicks  = 420;   // ~7s at 60fps
static const int   kCircleHoldTicks = 300;   // ~5s
static const int   kTransitionTicks = 90;    // ~1.5s

// Canvas child layout (fixed order → OnTick uses index arithmetic):
//   [0,         N*K)        tail segments  (orb i * kOrbTailLen + j)
//   [N*K,       N*K+N)      outer aura     (kOuterAuraBase + i)
//   [N*K+N,     N*K+2N)     inner glow     (kInnerGlowBase + i)
//   [N*K+2N,    N*K+3N)     core           (kCoreBase + i)
...
void OrbsBackground::LoadPalette()
{
    ...
    // glow [0], background [1]
    Color schemes[5][2] = {
        { {255, 30,160,255}, {255,  0,  8, 20} },  // electric (default)
        { {255,  0,220,150}, {255,  0, 10, 15} },  // aurora
        { {255,255,140,  0}, {255, 12,  5,  0} },  // solar
        { {255,200, 80,255}, {255,  8,  0, 20} },  // nebula
        { {255,255, 60,140}, {255, 15,  0, 12} },  // rose
    };
...
void OrbsBackground::InitOrbs()
{
    ...
    // Outer aura (soft bloom — large, low alpha)
    for (int i = 0; i < kOrbCount; ++i) {
        const auto& s = m_orbs[i];
    ...
    // Core — pure white so every orb reads as a bright hot point
    for (int i = 0; i < kOrbCount; ++i) {
        const auto& s = m_orbs[i];
        float x, y; OrbPos(s, x, y);
...
void OrbsBackground::OnTick(Object^ sender, Object^ args)
{
    ...
    // Mode hold countdown — flip target when timer expires
    if (--m_holdTicks <= 0) {
    ...
    // Advance lerp linearly toward target
    ...
    // Smoothstep easing on the blend value
    ...
    for (int i = 0; i < kOrbCount; ++i) {
        ...
        // Shift tail history (index 0 = oldest, kOrbTailLen-1 = newest)
        for (int j = 0; j < kOrbTailLen - 1; ++j) {
            s.tailX[j] = s.tailX[j + 1];
            s.tailY[j] = s.tailY[j + 1];
        }

        // Only advance the orbital angle when fully in dance mode.
        // Freezing it during transitions and the circle hold ensures the
        // return-to-dance blend always targets the same fixed orbital
        // position, eliminating the jolt caused by chasing a moving target.
        if (m_lerpT == 0.0f) s.angle += s.speed;
        float dx, dy;
        OrbPos(s, dx, dy);

        // Circle formation position — evenly distributed, slowly rotating
        ...
        // Blend between dance and circle
        ...
        // Update tail ellipse positions (size/opacity are baked at init)
        for (int j = 0; j < kOrbTailLen; ++j) {
            float t = static_cast<float>(j) / (kOrbTailLen - 1);
            float r = kMinTailR + t * (kMaxTailR - kMinTailR);
