...
// Prebuilt schemes: [0]=shape color, [1]=gradient top (light/rich), [2]=gradient bottom (deep/dark)
...
void SpheresBackground::LoadPalette()
{
    ...
    // custom: read shape + gradient-top; auto-derive gradient-bottom at ~40% brightness
                                   ...
                                   Windows::UI::Color shapeColor, float opacity)
{
    ...
    if (shapeType == kShapeTriangle) {
        // equilateral triangle inscribed in circumscribed circle of radius r, centered at (r,r) in local coords
