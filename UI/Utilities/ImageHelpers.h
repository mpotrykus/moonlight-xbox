...
// Place `src` centered on an opaque-black canvas padded by `padPx` on every side.
// Returns a new (srcW + 2*padPx) x (srcH + 2*padPx) BGRA8-Premultiplied bitmap.
Windows::Graphics::Imaging::SoftwareBitmap^ PadWithBlack(Windows::Graphics::Imaging::SoftwareBitmap^ src, unsigned int padPx);
