...
void TabsLayout::AnimateForeground(Button^ btn, bool selected)
{
    ...
    // Read the current color from whatever brush is already on the button so the
    // animation starts from wherever it is (handles interrupting a mid-fade).
    ...
    // Replace with a locally owned brush at the current color so the animation
    // doesn't accidentally modify a shared style resource.
    ...
    // Select: 300ms to roughly match when the indicator arrives.
    // Deselect: 150ms — indicator is already leaving, fade out quickly.
...
void TabsLayout::AnimateSelectionToButton(Button^ btn, bool animate)
{
    ...
    // Stop() reverts Y to the base (non-animated) value, which may be stale.
    // Restore it to where the last animation was heading before starting a new one.
    ...
    // BackEase: slight backward resistance at departure gives the "stuck at origin" feel.
...
void TabsLayout::SelectTabByName(Platform::String^ panelName)
{
    ...
    // Find the matching button first so we can skip early if already selected
    ...
    // Skip re-animating the already-active tab
    if (matchedButton != nullptr && matchedButton == m_selectedButton && m_selectionIndicatorReady) return;

    // Update button selection state and slide indicator
    ...
    // First load: swap immediately without animation
    if (!m_selectionIndicatorReady) {
    ...
    // Cancel any in-progress fade and reset opacity
    if (m_fadeStoryboard != nullptr) {
        m_fadeStoryboard->Stop();
        m_fadeStoryboard = nullptr;
    ...
    // Fade out (120ms), then swap visibility and fade in (200ms)
