...
SoftwareBitmap^ ImageHelpers::PadWithBlack(SoftwareBitmap^ src, unsigned int padPx) {
    if (src == nullptr || padPx == 0) return src;
    try {
        src = EnsureBgra8Premultiplied(src);
        unsigned int srcW = src->PixelWidth;
        unsigned int srcH = src->PixelHeight;
        unsigned int dstW = srcW + 2 * padPx;
        unsigned int dstH = srcH + 2 * padPx;

        auto outBmp = ref new SoftwareBitmap(BitmapPixelFormat::Bgra8, dstW, dstH, BitmapAlphaMode::Premultiplied);
        ...
        if (srcUnk && dstUnk &&
            SUCCEEDED(srcUnk->QueryInterface(IID_PPV_ARGS(&srcAccess))) &&
            SUCCEEDED(dstUnk->QueryInterface(IID_PPV_ARGS(&dstAccess))) &&
            SUCCEEDED(srcAccess->GetBuffer(&srcData, &srcCap)) &&
            SUCCEEDED(dstAccess->GetBuffer(&dstData, &dstCap)))
        {
            // Fill destination with opaque black
            for (unsigned int y = 0; y < dstH; ++y) {
                uint8_t* row = dstData + dstDesc.StartIndex + y * dstDesc.Stride;
                for (unsigned int x = 0; x < dstW; ++x) {
                    row[x*4+0] = 0; row[x*4+1] = 0; row[x*4+2] = 0; row[x*4+3] = 255;
                }
            }
            // Copy source rows into the padded center
            for (unsigned int y = 0; y < srcH; ++y) {
                uint8_t* srcRow = srcData + srcDesc.StartIndex + y * srcDesc.Stride;
                uint8_t* dstRow = dstData + dstDesc.StartIndex + (y + padPx) * dstDesc.Stride + padPx * 4;
                memcpy(dstRow, srcRow, (size_t)srcW * 4);
            }
            return outBmp;
        }

        // Fallback path via DataReader/DataWriter
        std::vector<uint8_t> srcBytes((size_t)srcH * srcDesc.Stride);
        try {
            auto ib = ref new Windows::Storage::Streams::Buffer((unsigned int)srcBytes.size());
            src->CopyToBuffer(ib);
            auto reader = Windows::Storage::Streams::DataReader::FromBuffer(ib);
        ...
        std::vector<uint8_t> dstTmp((size_t)dstH * dstDesc.Stride, 0);
        for (unsigned int y = 0; y < dstH; ++y) {
            uint8_t* row = dstTmp.data() + y * dstDesc.Stride;
            for (unsigned int x = 0; x < dstW; ++x) {
                row[x*4+0] = 0; row[x*4+1] = 0; row[x*4+2] = 0; row[x*4+3] = 255;
            }
        }
        for (unsigned int y = 0; y < srcH; ++y) {
            uint8_t* srcRow = srcBytes.data() + y * srcDesc.Stride;
            uint8_t* dstRow = dstTmp.data() + (y + padPx) * dstDesc.Stride + padPx * 4;
            memcpy(dstRow, srcRow, (size_t)srcW * 4);
        }
        try {
            auto writer = ref new Windows::Storage::Streams::DataWriter();
